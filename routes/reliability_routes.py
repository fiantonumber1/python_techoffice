...
def calculate_reliability(fungsi_str, lambdas, t_values):
    ...
    def symbolic_direct():
        ...
        return {"h_str": str(h_expr), "f_str": str(f_expr), "rows": local_rows, "method": "symbolic"}
    ...
    def symbolic_log():
        ...
        return {"h_str": str(h_expr), "f_str": str(f_expr), "rows": local_rows, "method": "symbolic_log"}

    try:
        print("Trying symbolic direct method...")
        result = symbolic_direct()
        h_str, f_str, rows, method = result["h_str"], result["f_str"], result["rows"], result["method"]
    except Exception as e:
        ...
        for t_val in t_values:
            try:
                ...
                if R_t >= 1.0:
                    ...
                    R_plus = float(R_func(t_val + delta))
                    R_minus = float(R_func(max(t_val - delta, 1e-12)))
...
def invert_by_low_order_taylor(r_target, R_t_str, order=2, do_subs=None):
    ...
    symbol_map = {}
    for name in found_lam.union(found_caps):
    ...
    if do_subs is not None and best_sol is not None:
        try:
            ...
            expr_result = best_sol.subs(full_subs)
            print(f"[DEBUG] setelah subs: {expr_result}")
            free_syms = expr_result.free_symbols
            if free_syms:
                print(f"[ERROR] Simbol belum diganti: {free_syms}")
            else:
                ev = expr_result.evalf()
    ...
    return {'t_expression': best_sol, 't_value': t_value}


# ── Routes ───────────────────────────────────────────────────────────────────

@reliability_bp.route('/calculate_hazard', methods=['POST'])
def calc():
...
@reliability_bp.route('/calculate_t_r', methods=['POST'])
def calc_t_r():
    data = request.get_json()
    if not data:
        return jsonify({"error": "No JSON"}), 400
    ...
    try:
        ...
        if 0 < r_target_to_find_t < 1:
            ...
            try:
                ...
                result['t_for_R'] = {'R_target': r_target_to_find_t, 'error': f"Inversion failed: {str(e_inv)}"}
        else:
